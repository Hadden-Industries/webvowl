...
Operations use domain verbs that describe their observable effect. Boolean state fields use a positive predicate such as `isFocusable`, `isRetryable`, or `hasEnded`; imperative request flags may use a positive verb such as `includeNeighborhood`. Quantities encode their unit or measure when it is not inherent in the type, such as `settleTimeoutMs`, `maxInlineDocumentBytes`, `maxToolNameCharacters`, `byteLength`, `widthPx`, and `heightPx`. Lifecycle-sensitive names identify the lifetime they belong to, such as `loadGeneration`, `pageLocalArtifactId`, or `objectUrl`.
