...
from _configuration_transaction import (
    ...
    ensure_generated_setup_root_is_safe,
...
def remove_managed_block(text: str, name: str) -> str:
    begin = f"# BEGIN {name} - managed by util/set_up_sdlc.py"
    end = f"# END {name} - managed by util/set_up_sdlc.py"
...
def managed_block(name: str, body: str) -> str:
    return (f"# BEGIN {name} - managed by util/set_up_sdlc.py\n"
            + body.strip() + "\n"
            + f"# END {name} - managed by util/set_up_sdlc.py\n")
...
def sdlc_stop_hook(source: Path | None = None) -> dict:
    # The shared native bootstrap roots a Node entry point in this Git checkout.
