...
import { createNTriplesSyntaxAdapter } from "../src/owlapi-js/parser/rdf/n3SyntaxAdapter.js";
...
const fixtures = Object.freeze({
  ...
  nTriplesFirstUse: generateBenchmarkFixture("ntriples", { count: 100 }),
  nTriplesLarge: generateBenchmarkFixture("ntriples", { count: 50_000 }),
});
const sources = Object.freeze({
  firstUse: new StringDocumentSource(fixtures.nTriplesFirstUse, {
    documentIRI: "urn:owlapi-js:benchmark:ntriples:first-use",
    fileName: "first-use.nt",
  }),
  functional: new StringDocumentSource(fixtures.functionalLarge),
  mismatch: new StringDocumentSource(fixtures.mismatchLarge),
  ntriples: new StringDocumentSource(fixtures.nTriplesLarge, {
    documentIRI: "urn:owlapi-js:benchmark:ntriples:large",
    fileName: "generated-ntriples-large.nt",
  }),
});
...
const phase12Descriptors = Object.freeze([
  owlXmlParserDescriptor,
  rdfXmlParserDescriptor,
...
const syntaxOnly = async (chunkSize) => {
  const { dataset } = await createNTriplesSyntaxAdapter({ chunkSize }).parse(
    sources.ntriples,
  );
  if (dataset.size === 0) {
    throw new Error("The N-Triples syntax benchmark produced no quads");
  }
};
...
const parseWithDescriptors = async (source, descriptors) => {
  ...
  if (ontology.getAxioms().size === 0) {
    throw new Error("The N-Triples benchmark produced no OWL axioms");
  }
};
...
const rejectMismatch = async (descriptors) => {
  ...
  throw new Error("The N-Triples mismatch benchmark unexpectedly parsed");
};

const firstUse = await sample(() =>
  parseWithDescriptors(sources.firstUse, phase12Descriptors),
);
const benchmarks = Object.freeze([
  ...[16_384, 65_536, 262_144].map((chunkSize) => ({
    chunkSize,
    id: `generated-ntriples-large.syntax-to-rdf.chunk-${chunkSize}`,
    operation: () => syntaxOnly(chunkSize),
  })),
  {
    chunkSize: 65_536,
    id: "generated-ntriples-large.end-to-end",
    operation: () => parseWithDescriptors(sources.ntriples, phase12Descriptors),
  },
  {
    id: "generated-functional-large-phase11-control",
    operation: () =>
      parseWithDescriptors(sources.functional, phase11Descriptors),
  },
  {
    id: "generated-functional-large-phase12",
    operation: () =>
      parseWithDescriptors(sources.functional, phase12Descriptors),
  },
  {
    id: "generated-mismatch-large-phase11-control",
    operation: () => rejectMismatch(phase11Descriptors),
  },
  {
    id: "generated-mismatch-large-phase12",
  ...
  JSON.stringify(
    {
      ...
      firstUse: {
        inputBytes: Buffer.byteLength(fixtures.nTriplesFirstUse),
        measurement: firstUse,
        path: "manager -> first conditional N3.js load -> N-Triples -> shared RDF-to-OWL",
      },
      measuredOn: new Date().toISOString(),
      protocol: {
        aggregation: "median",
        fixture: {
          bytes: Buffer.byteLength(fixtures.nTriplesLarge),
          count: 50_000,
          generator: GENERATOR_VERSION,
          id: "generated-ntriples-large",
        },
