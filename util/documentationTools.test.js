...
import { prepareDocumentationTools } from "./prepareDocumentationTools.mjs";

let root;
const integrity = `sha512-${Buffer.alloc(64, 7).toString("base64")}`;
const wheelHash = "a".repeat(64);
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), "webvowl-documentation-tools-"));
  writeFileSync(
...
afterEach(() => {
  if (
    ...
    !basename(root).startsWith("webvowl-documentation-tools-")
  ) {
    throw new Error(
