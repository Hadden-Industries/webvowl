...
import { KRSSLexer } from "../krss/lexer.js";
...
const documentNamespace = (source) => {
  ...
  if (!documentIri) {
    return `urn:owlapi-js:krss2-document:${++anonymousDocumentSequence}#`;
  }
...
export class OWLKRSS2SyntaxOWLParser {
  #configuration;
  #dataFactory;
  #lastYieldAt;
  #lexer;
  ...
  async parse(source, transaction, configuration) {
    ...
    while (this.#lexer.peek().type !== "EOF") {
      if (this.#acceptKeyword("end-tbox")) {
        if (this.#tboxClosed || this.#seenABox) {
          this.#syntax(
            "The KRSS2 TBox delimiter is out of place",
            this.#lexer.peek(),
            { found: "end-tbox" },
          );
        }
      ...
      if (this.#acceptKeyword("end-abox")) {
        ...
        if (trailing.type !== "EOF") {
          ...
          this.#syntax(
            "No KRSS2 statement may follow the ABox delimiter",
            trailing,
            { found },
          );
        }
    ...
    this.#expect("EOF");
    transaction.setDocumentFormat(OWLDocumentFormats.KRSS2);
    return OWLDocumentFormats.KRSS2;
  }
  ...
  #parseTopLevel() {
    ...
    if (ABOX_OPERATIONS.has(keyword)) {
      ...
      // OWLAPI's document production is TBox* followed by ABox*. Enforcing the
      // boundary prevents accepting files the public KRSS2 parser rejects.
      this.#syntax(
        "A KRSS2 TBox statement cannot follow an ABox statement",
        operation,
        {
    ...
    switch (keyword) {
      ...
      default:
        this.#syntax("Unsupported KRSS2 top-level production", operation, {
          found: operation.value,
        });
    }
  }
  ...
  #parsePrimitiveRole() {
    ...
    while (this.#lexer.peek().type !== ")") {
      ...
      if (attributeOrder === undefined) {
        this.#syntax("Unsupported KRSS2 primitive-role attribute", attribute, {
          found: `:${attribute.value}`,
        });
      }
      if (attributeOrder < previousAttributeOrder) {
        this.#syntax(
          "KRSS2 primitive-role attributes are out of order",
          attribute,
          {
            found: `:${attribute.value}`,
          },
      ...
      if (seenAttributes.has(name)) {
        this.#syntax("A KRSS2 role attribute cannot be repeated", attribute, {
          found: `:${attribute.value}`,
        });
      }
      if (
        ...
        this.#syntax(
          "Mutually exclusive KRSS2 role attributes cannot be combined",
          attribute,
          { found: `:${attribute.value}` },
        );
      }
      ...
      switch (name) {
        ...
        case "inverse":
          ...
          break;
        default:
          throw new TypeError("Unreachable KRSS2 primitive-role attribute");
      }
    }
    this.#expect(")");
    return axioms;
  ...
  #parseParentsAttribute(property, axioms) {
    ...
    if (parentCount === 0) {
      this.#syntax("A KRSS2 parents attribute cannot be empty", closing);
    }
  }
  ...
  #parseAttributeClassExpressions() {
    ...
    const closing = this.#expect(")");
    if (values.length === 0) {
      this.#syntax("A KRSS2 role attribute list cannot be empty", closing);
    }
  ...
  #parseBoolean() {
    ...
    this.#syntax("A KRSS2 role flag must be t or nil", token, {
      expected: "t or nil",
      found: token.value,
    });
  }
  ...
  #parsePropertyChain() {
    ...
    if (operation.value.toLowerCase() !== "compose") {
      this.#syntax("Expected the KRSS2 compose role operator", operation, {
        expected: "compose",
        found: operation.value,
      });
    }
  ...
  #parseClassExpression(depth) {
    ...
    switch (operation.value.toLowerCase()) {
      ...
      default:
        this.#syntax("Unsupported KRSS2 class expression", operation, {
          found: operation.value,
        });
    }
  }
  ...
  #parseNaryExpression(depth, kind) {
    ...
    if (unique.length < 2) {
      this.#syntax("A KRSS2 Boolean expression requires two operands", closing);
    }
  ...
  #parseCardinalityRestriction(depth, kind) {
    ...
    if (!Number.isSafeInteger(cardinality)) {
      this.#syntax(
        "The KRSS2 cardinality is outside the safe integer range",
        cardinalityToken,
  ...
  #parseObjectPropertyExpression() {
    ...
    if (operation.value.toLowerCase() !== "inv") {
      this.#syntax("Expected the KRSS2 inverse role operator", operation, {
        expected: "inv",
        found: operation.value,
      });
    }
  ...
  #expectName() {
    const token = this.#lexer.peek();
    if (!this.#isNameToken(token)) {
      this.#syntax("Expected a KRSS2 entity name", token, {
    ...
    if (
      ...
      this.#syntax("A reserved KRSS2 keyword cannot be an entity name", token, {
        found: token.value,
      });
    }
    return this.#lexer.consume();
  ...
  #isNameToken(token) {
    return ["SYMBOL", "FULL_IRI", "INTEGER"].includes(token.type);
  }
  ...
  #checkExpressionDepth(depth) {
    ...
    throw new ResourceLimitError(
      "The KRSS2 expression depth limit was exceeded",
      {
        ...this.#location(token),
  ...
  #expect(type) {
    const token = this.#lexer.peek();
    if (token.type !== type) {
      this.#syntax(`Expected KRSS2 token ${type}`, token, {
        expected: type,
        found: token.value || token.type,
      });
    }
