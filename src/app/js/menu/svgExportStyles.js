...
module.exports = function createExportSvgClone( liveSvg, getComputedStyle ){
  const readComputedStyle = getComputedStyle || window.getComputedStyle.bind(window);
  const exportedSvg = liveSvg.cloneNode(true);
  const liveElements = [liveSvg].concat(Array.from(liveSvg.querySelectorAll("*")));
  const exportedElements = [exportedSvg].concat(Array.from(exportedSvg.querySelectorAll("*")));

  liveElements.forEach(function ( liveElement, index ){
    const exportedElement = exportedElements[index];
    if ( !exportedElement || !exportedElement.style ) {return;}
