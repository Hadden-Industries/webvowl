...
CenteringTextElement.prototype.saveCorrespondingSpan = function (
  correspondingSpan,
) {
...
CenteringTextElement.prototype.updateAllTextElements = function () {
  ...
  for (let i = 0; i < this.storedSpanArrays.length; i++) {
    const truncatedText = textTools.truncate(
      ...
      this._textBlock().datum().textWidth(),
      this.storedStyle[i],
    );
...
CenteringTextElement.prototype.addTextline = function (
  text,
  style,
  prefix,
  postfix,
