  ...
  *tokenize() {
    ...
    while (this.pos < length) {
      ...
      if (ch === '"' || ch === "'") {
        ...
        while (
          this.pos < length &&
          !skip.has(this.input[this.pos]) &&
          !delims.has(this.input[this.pos]) &&
          !commentDelimiters.has(this.input[this.pos])
        ) {
          sb += this.input[this.pos++];
        }
        yield* consumeToken();
...
export const df = {
  ...
  getOWLLiteral: (val, lang, datatype) => ({
    type: "Literal",
    val,
    lang,
    datatype,
...
export class ManchesterParser {
  ...
  parseFrames(keyword) {
    ...
    if (
      kwLower === "class:" ||
      kwLower === "objectproperty:" ||
      kwLower === "dataproperty:" ||
      kwLower === "annotationproperty:" ||
      kwLower === "individual:" ||
      kwLower === "datatype:" ||
      kwLower === "valuepartition:"
    ) {
      ...
      while (this.peekToken() !== "|EOF|") {
        ...
        if (
          ...
          next === "inverses:" ||
          next === "superclassof:" ||
          next === "superpropertyof:" ||
          next === "disjointunionof:" ||
          next === "haskey:" ||
          next === "subpropertychain:"
        ) {
  ...
  parseRuleFrame() {
    ...
    if (
      this.peekToken() !== "|EOF|" &&
      !isManchesterSyntaxFormat(this.peekToken())
    ) {
  ...
  parseSwrlArgument() {
    ...
    if (tok.startsWith("?")) {
      ...
      !isNaN(tok) ||
      ["true", "false", "true:", "false:"].includes(tok.toLowerCase())
    ) {
      return this.parseLiteral();
    } else {
  ...
  parseIntersection() {
    ...
    while (
      this.peekToken().toLowerCase() === "and" ||
      this.peekToken().toLowerCase() === "that"
    ) {
      this.consumeToken();
  ...
  parseNonNary() {
    ...
    if (
      ["some", "only", "onlysome", "value", "min", "max", "exactly"].includes(
        tokenAhead,
      )
    ) {
...
export class TriplesEmitter {
  ...
  emit(ast) {
    if (this.baseIri) {
      this.add(
        this.getURI(`<${this.baseIri}>`),
        "rdf:type",
        this.getURI("owl:Ontology"),
      );
    }
    ...
    for (const node of ast) {
      if (node.type === "Frame") {
        ...
        if (kwLower === "objectproperty:") {
          this.knownObjectProperties.add(node.subject);
        }
        if (kwLower === "dataproperty:") {
          this.knownDataProperties.add(node.subject);
        }
      }
    }
    ...
    for (const node of ast) {
      if (node.type === "Frame") {
        ...
        if (kwLower === "class:") {
          rdfType = "owl:Class";
        } else if (kwLower === "objectproperty:") {
          rdfType = "owl:ObjectProperty";
        } else if (kwLower === "dataproperty:") {
          rdfType = "owl:DatatypeProperty";
        } else if (kwLower === "annotationproperty:") {
          rdfType = "owl:AnnotationProperty";
        } else if (kwLower === "individual:") {
          rdfType = "owl:NamedIndividual";
        } else if (kwLower === "datatype:") {
          rdfType = "rdfs:Datatype";
        }
        ...
        for (const item of node.items) {
          ...
          if (itemKw === "subclassof:") {
            predicate = "rdfs:subClassOf";
          } else if (itemKw === "superclassof:") {
            predicate = "owl:hasSubClass";
          } // non-standard but often used or inverted
          else if (itemKw === "equivalentto:") {
            predicate = "owl:equivalentClass";
          } else if (itemKw === "disjointwith:") {
            predicate = "owl:disjointWith";
          } else if (itemKw === "disjointunionof:") {
            predicate = "owl:disjointUnionOf";
          } else if (itemKw === "subpropertyof:") {
            predicate = "rdfs:subPropertyOf";
          } else if (itemKw === "superpropertyof:") {
            predicate = "owl:hasSubProperty";
          } else if (itemKw === "domain:") {
            predicate = "rdfs:domain";
          } else if (itemKw === "range:") {
            predicate = "rdfs:range";
          } else if (itemKw === "types:") {
            predicate = "rdf:type";
          } else if (itemKw === "sameas:") {
            predicate = "owl:sameAs";
          } else if (itemKw === "differentfrom:") {
            predicate = "owl:differentFrom";
          } else if (itemKw === "inverseof:" || itemKw === "inverses:") {
            predicate = "owl:inverseOf";
          } else if (itemKw === "haskey:") {
            predicate = "owl:hasKey";
          } else if (itemKw === "subpropertychain:") {
            predicate = "owl:propertyChainAxiom";
          }

          if (
            itemKw === "disjointunionof:" ||
            itemKw === "haskey:" ||
            itemKw === "subpropertychain:"
          ) {
            ...
            for (const listExpr of item.expressions) {
              if (listExpr.type === "PropertyChain") {
                ...
                this.add(
                  ...
                  predicate,
                  this.emitList(
                    chainList.map((u) => ({ type: "NamedIndividual", iri: u })),
                  ),
                ); // emitList expects AST nodes or will pass objects
              } else {
            ...
            for (const fact of item.expressions) {
              ...
              if (fact.isNegative) {
                ...
                this.add(
                  bnode,
                  "rdf:type",
                  this.getURI("owl:NegativePropertyAssertion"),
                );
                this.add(bnode, "owl:sourceIndividual", subject);
                this.add(
                  bnode,
                  "owl:assertionProperty",
                  this.getURI(fact.property),
                );
            ...
            for (const charExpr of item.expressions) {
              ...
              if (charIri === "functional") {
                this.add(
                  subject,
                  "rdf:type",
                  this.getURI("owl:FunctionalProperty"),
                );
              } else if (charIri === "inversefunctional") {
                this.add(
                  subject,
                  "rdf:type",
                  this.getURI("owl:InverseFunctionalProperty"),
                );
              } else if (charIri === "symmetric") {
                this.add(
                  subject,
                  "rdf:type",
                  this.getURI("owl:SymmetricProperty"),
                );
              } else if (charIri === "transitive") {
                this.add(
                  subject,
                  "rdf:type",
                  this.getURI("owl:TransitiveProperty"),
                );
              } else if (charIri === "reflexive") {
                this.add(
                  subject,
                  "rdf:type",
                  this.getURI("owl:ReflexiveProperty"),
                );
              } else if (charIri === "irreflexive") {
                this.add(
                  subject,
                  "rdf:type",
                  this.getURI("owl:IrreflexiveProperty"),
                );
              } else if (
                charIri === "asymmetric" ||
                charIri === "antisymmetric"
              ) {
                this.add(
                  subject,
                  "rdf:type",
                  this.getURI("owl:AsymmetricProperty"),
                );
              }
            }
          } else {
        ...
        const kwLower = node.keyword.toLowerCase();
        let rdfType = null;
        if (kwLower === "disjointclasses:") {
          rdfType = "owl:AllDisjointClasses";
        } else if (kwLower === "equivalentclasses:") {
          rdfType = "owl:EquivalentClasses";
        } else if (kwLower === "equivalentproperties:") {
          rdfType = "owl:EquivalentProperties";
        } else if (kwLower === "disjointproperties:") {
          rdfType = "owl:AllDisjointProperties";
        } else if (kwLower === "sameindividual:") {
          rdfType = "owl:SameIndividual";
        } else if (kwLower === "differentindividuals:") {
          rdfType = "owl:AllDifferent";
        }
  ...
  emitExpression(expr) {
    ...
    if (expr.type === "SwrlAtom") {
      ...
      const isBuiltin =
        predStr.startsWith("swrlb:") ||
        predStr.startsWith("http://www.w3.org/2003/11/swrlb#");
      ...
      if (isBuiltin) {
        ...
        if (
          this.knownDataProperties &&
          this.knownDataProperties.has(expr.predicate)
        ) {
    ...
    if (expr.type === "OneOf") {
      const bnode = this.newBNode();
      this.add(bnode, "rdf:type", this.getURI("owl:Class"));
      const individuals = expr.individuals.map((iri) => ({
    ...
    if (expr.type === "DatatypeWithRestrictions") {
      ...
      for (const facet of expr.restrictions.facets) {
        ...
        if (facet.facetType === ">=") {
          facetIri = "xsd:minInclusive";
        } else if (facet.facetType === ">") {
          facetIri = "xsd:minExclusive";
        } else if (facet.facetType === "<=") {
          facetIri = "xsd:maxInclusive";
        } else if (facet.facetType === "<") {
          facetIri = "xsd:maxExclusive";
        } else if (facet.facetType === "length") {
          facetIri = "xsd:length";
        } else if (facet.facetType === "minLength") {
          facetIri = "xsd:minLength";
        } else if (facet.facetType === "maxLength") {
          facetIri = "xsd:maxLength";
        } else if (facet.facetType === "pattern") {
          facetIri = "xsd:pattern";
        } else if (facet.facetType === "langRange") {
          facetIri = "rdf:langRange";
        }

        const rNode = this.newBNode();
        this.add(rNode, facetIri, {
          type: "LITERAL",
          value: facet.facetValue.val,
          datatype: facet.facetValue.datatype
            ? this.getURI(facet.facetValue.datatype)
            : null,
          lang: facet.facetValue.lang,
        });
      ...
      this.add(
        ...
        this.emitList(
          restrictionsList.map((n) => ({ type: "RawBNode", id: n })),
        ),
      );
      return bnode;
    }
    ...
    if (expr.type === "Restriction") {
      if (expr.restrictionType === "onlysome") {
        ...
        this.add(
          someNode,
          "owl:someValuesFrom",
          this.emitExpression(expr.filler),
        );
        ...
        this.add(
          ...
          "owl:intersectionOf",
          this.emitList([
            { type: "RawBNode", id: someNode },
            { type: "RawBNode", id: onlyNode },
          ]),
        );
        return interNode;
      }
      ...
      if (expr.filler && expr.filler.type === "Self") {
        this.add(bnode, "owl:hasSelf", {
          type: "LITERAL",
          value: "true",
          datatype: this.getURI("xsd:boolean"),
        });
      } else {
        ...
        if (expr.restrictionType === "some") {
          ...
          this.add(bnode, "owl:minQualifiedCardinality", {
            type: "LITERAL",
            value: expr.cardinality.toString(),
            datatype: this.getURI("xsd:nonNegativeInteger"),
          });
          if (fillerNode) {
            this.add(bnode, "owl:onClass", fillerNode);
          }
        } else if (expr.restrictionType === "max") {
          this.add(bnode, "owl:maxQualifiedCardinality", {
            type: "LITERAL",
            value: expr.cardinality.toString(),
            datatype: this.getURI("xsd:nonNegativeInteger"),
          });
          ...
          this.add(bnode, "owl:qualifiedCardinality", {
            type: "LITERAL",
            value: expr.cardinality.toString(),
            datatype: this.getURI("xsd:nonNegativeInteger"),
          });
...
export function isManchesterSyntaxFormat(text) {
  ...
  return /^\s*(Prefix:|Ontology:|Class:|ObjectProperty:|DataProperty:|Individual:|AnnotationProperty:|Datatype:|Rule:|ValuePartition:)/i.test(
    snippet,
  );
}

export function convertManchesterSyntaxToRdfXml(
  text,
