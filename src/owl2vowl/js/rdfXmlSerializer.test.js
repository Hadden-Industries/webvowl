...
describe("rdfXmlSerializer", () => {
  ...
  it("should serialize basic URI triples into rdf:Description blocks with namespaces", () => {
    ...
    const xml = serializeTriplesToRdfXml(triples, prefixes, "http://example.org/test");
    ...
    expect(descriptions[0].getAttribute("rdf:about")).toBe("http://example.org/test#Subject");

    const types = doc.getElementsByTagName("rdf:type");
    expect(types.length).toBe(1);
    expect(types[0].getAttribute("rdf:resource")).toBe("http://www.w3.org/2002/07/owl#Class");
  });
  ...
  it("should handle blank nodes (BNODE) using rdf:nodeID and strip leading '_:' if present", () => {
    const triples = [
      {
        ...
        predicate: { type: "IRI", value: "http://www.w3.org/1999/02/22-rdf-syntax-ns#type" },
        object: { type: "URI", value: "http://www.w3.org/2002/07/owl#Restriction" }
      },
      {
        subject: { type: "URI", value: "http://example.org/test#ClassA" },
        predicate: { type: "IRI", value: "http://www.w3.org/2000/01/rdf-schema#subClassOf" },
    ...
    for (let i = 0; i < descriptions.length; i++) {
      if (descriptions[i].getAttribute("rdf:nodeID") === "_:b1" || descriptions[i].getAttribute("rdf:nodeID") === "b1") {
        bnodeDescFound = true;
      }
    }
  ...
  it("should serialize literal objects with language tags and datatypes", () => {
    const triples = [
      {
        ...
        predicate: { type: "IRI", value: "http://www.w3.org/2000/01/rdf-schema#label" },
        object: { type: "LITERAL", value: "Hello World", lang: "en" }
      },
      {
        subject: { type: "URI", value: "http://example.org/test#Thing" },
        predicate: { type: "IRI", value: "http://example.org/test#count" },
        object: { type: "LITERAL", value: "42", datatype: { type: "URI", value: "http://www.w3.org/2001/XMLSchema#integer" } }
      }
    ];
    const xml = serializeTriplesToRdfXml(triples, { ex: "http://example.org/test#" }, "");
    ...
    expect(counts[0].getAttribute("rdf:datatype")).toBe("http://www.w3.org/2001/XMLSchema#integer");
  });
    ...
    const triples = [
      {
        ...
        predicate: { type: "IRI", value: "http://custom-domain.com/vocab#customProp" },
