...
module.exports = (function () {
  const o = function (graph) {
    ...
    this.actualRadius = function () {
      if (
        !graph.options().scaleNodesByIndividuals() ||
        that.individuals().length <= 0
      ) {
        return that.radius();
      } else {
        ...
        const MULTIPLIER = 8,
          additionalRadius =
            Math.log(that.individuals().length + 1) * MULTIPLIER + 5;
    ...
    this.drawHalo = function (pulseAnimation) {
      ...
      if (rectangularRepresentation === true) {
        haloGroupElement = drawTools.drawRectHalo(
          that.nodeElement(),
          80,
          80,
          5,
        ...
        haloGroupElement = drawTools.drawHalo(
          that.nodeElement(),
          that.actualRadius(),
          this.removeHalo,
        );
    ...
    this.draw = function (parentElement, additionalCssClasses) {
      ...
      if (rectangularRepresentation === true) {
        renderingElement = drawTools.appendRectangularClass(
          parentElement,
          80,
          80,
          cssClasses,
        ...
        renderingElement = drawTools.appendCircularClass(
          parentElement,
          that.actualRadius(),
          cssClasses,
          that.labelForCurrentLanguage(),
