module.exports = function (graph) {
  ...
  Domain_dragger.updateDomain = function (newDomain) {
    if (
      graph.genericPropertySanityCheck(
        Domain_dragger.parent.range(),
        newDomain,
        Domain_dragger.parent.type(),
  ...
  Domain_dragger.setParentProperty = function (parentProperty, inverted) {
    ...
    if (inverted === true) {
      ...
      if (
        ...
        parentProperty.labelElement().attr("transform") === "translate(0,15)"
      ) {
  ...
  Domain_dragger.hideDragger = function (val) {
    if (Domain_dragger.pathElement) {
      Domain_dragger.pathElement.classed("hidden", val);
    }
    if (Domain_dragger.nodeElement) {
      Domain_dragger.nodeElement.classed("hidden", val);
    }
    if (Domain_dragger.draggerObject) {
      Domain_dragger.draggerObject.classed("hidden", val);
    }
  };
  ...
  Domain_dragger.drawNode = function () {
    Domain_dragger.pathElement = Domain_dragger.pathLayer
      .append("line")
      .classed("classNodeDragPath", true);
    Domain_dragger.pathElement
      .attr("x1", 0)
      .attr("y1", 0)
      .attr("x2", 0)
      .attr("y2", 0);
    ...
    const pathData =
      "M 10,40 C -10,15 -10,-15 10,-40 -8.8233455,-13.641384 -36.711107,-5.1228436 -50,0 -36.696429,4.9079017 -8.6403157,13.745728 10,40 Z";
    Domain_dragger.nodeElement = Domain_dragger.rootNodeLayer
      .append("path")
      .attr("d", pathData);
    Domain_dragger.nodeElement.classed("classDraggerNode", true);
    if (graph.options().useAccuracyHelper()) {
      Domain_dragger.draggerObject =
        Domain_dragger.rootNodeLayer.append("circle");
      Domain_dragger.draggerObject
        .attr("r", 40)
        .attr("cx", 0)
        .attr("cy", 0)
        .classed("superHiddenElement", true);
      Domain_dragger.draggerObject.classed(
        "superOpacityElement",
  ...
  Domain_dragger.updateElementViaRangeDragger = function (x, y) {
    ...
    const angle =
      (Math.atan2(ep_range_y - range_y, ep_range_x - range_x) * 180) / Math.PI;

    Domain_dragger.nodeElement.attr(
      "transform",
      "translate(" +
        ep_range_x +
        "," +
    ...
    Domain_dragger.draggerObject.attr(
      "transform",
      "translate(" + dox + "," + doy + ")",
    );
  };
  ...
  Domain_dragger.addMouseEvents = function () {
    ...
    rootLayer
      .on("mouseover", Domain_dragger.onMouseOver)
  ...
  Domain_dragger.mouseEntered = function (p) {
    if (!arguments.length) {
      return Domain_dragger.mouseEnteredVar;
    }
  ...
  Domain_dragger.setPosition = function (x, y) {
    ...
    const angle =
      (Math.atan2(range_y - ep_range_y, range_x - ep_range_x) * 180) / Math.PI +
      180;
