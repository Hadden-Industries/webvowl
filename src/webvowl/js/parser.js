...
module.exports = function (graph) {
  ...
  parser.parse = function (ontologyData) {
    ...
    const classes = combineClasses(
      ontologyData.class,
      ontologyData.classAttribute,
    );
    const datatypes = combineClasses(
      ontologyData.datatype,
      ontologyData.datatypeAttribute,
    );
    const combinedClassesAndDatatypes = classes.concat(datatypes);
    ...
    mergeRangesOfEquivalentProperties(
      combinedProperties,
        ...
        if (Prototype) {
          ...
          if (element.individuals) {
            element.individuals.forEach(function (individual) {
              ...
              individualNode.label(individual.labels).iri(individual.iri);
  ...
  function mergeRangesOfEquivalentProperties(properties, nodes) {
    ...
    const newNodes = equivalentPropertyMerger.merge(
      properties.slice(),
      nodes.slice(),
      propertyMap,
      classMap,
  ...
  function createPropertyStructure(rawProperties, classMap, propertyMap) {
    ...
    // Connect properties
    rawProperties.forEach(function (property) {
      let domain, range, domainObject, rangeObject, inverse;
      ...
      if ((property.domain() && property.range()) || property.inverse()) {
        ...
        if (inversePropertyId) {
          ...
          if (!inverse) {
            console.warn(
              "No inverse property was found for id: " + inversePropertyId,
            );
        ...
        if (
          ...
          console.warn(
            "Domain and range not found for property: " + property.id(),
          );
        }
  ...
  function referenceSubOrSuperProperties(subOrSuperPropertiesArray) {
    ...
    for (i = 0, l = subOrSuperPropertiesArray.length; i < l; ++i) {
      ...
      if (subOrSuperProperty) {
        ...
        console.warn(
          "No sub-/superproperty was found for id: " + subOrSuperPropertyId,
        );
      }
    }
  }
  ...
  function addAdditionalAttributes(base, addition) {
    ...
    for (const addAttribute in addition) {
      ...
      if (
        ...
        Object.prototype.hasOwnProperty.call(addition, addAttribute)
      ) {
