module.exports = function (graph) {
  ...
  Class_dragger.setParentNode = function (parentNode) {
    ...
    if (Class_dragger.mouseButtonPressed === false) {
      if (
        Class_dragger.parent.actualRadius &&
        Class_dragger.parent.actualRadius()
      ) {
        Class_dragger.x =
          Class_dragger.parent.x + 10 + Class_dragger.parent.actualRadius();
        Class_dragger.y =
          Class_dragger.parent.y + 10 + Class_dragger.parent.actualRadius();
      } else {
  ...
  Class_dragger.hideDragger = function (val) {
    ...
    if (Class_dragger.nodeElement) {
      Class_dragger.nodeElement.classed("hidden", val);
    }
    if (Class_dragger.draggerObject) {
      Class_dragger.draggerObject.classed("hidden", val);
    }
  };
  ...
  Class_dragger.drawNode = function () {
    Class_dragger.pathElement = Class_dragger.pathLayer
      .append("line")
      .classed("classNodeDragPath", true);
    Class_dragger.pathElement
      .attr("x1", 0)
      .attr("y1", 0)
      .attr("x2", 0)
      .attr("y2", 0);
    ...
    Class_dragger.nodeElement = Class_dragger.rootNodeLayer
      .append("path")
      .attr("d", pathData);
    ...
    if (graph.options().useAccuracyHelper()) {
      Class_dragger.draggerObject
        .attr("r", 40)
        .attr("cx", 0)
        .attr("cy", 0)
        .classed("superHiddenElement", true);
      Class_dragger.draggerObject.classed(
        "superOpacityElement",
  ...
  Class_dragger.updateElement = function () {
    ...
    if (Class_dragger.pathElement) {
      ...
      const ncx = nX * 15;
      const ncy = nY * 15;
      Class_dragger.draggerObject.attr("cx", ncx).attr("cy", ncy);
    ...
    const angle =
      (Math.atan2(
        Class_dragger.parent.y - Class_dragger.y,
        Class_dragger.parent.x - Class_dragger.x,
      ) *
        180) /
      Math.PI;

    Class_dragger.nodeElement.attr(
      "transform",
      "translate(" +
        Class_dragger.x +
        "," +
    ...
    Class_dragger.draggerObject.attr(
      "transform",
  ...
  Class_dragger.mouseEntered = function (p) {
    if (!arguments.length) {
      return Class_dragger.mouseEnteredVar;
    }
