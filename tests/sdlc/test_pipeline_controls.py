...
sys.path.insert(0, str(REPOSITORY / 'util'))
import sdlc
...
class LocalVerificationControlTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        shutil.copytree(REPOSITORY / 'util', self.repo / 'util',
        ...
        self.git('remote', 'add', 'origin', 'https://github.com/example/service.git')
    ...
    def test_begin_requires_explicit_functionality_declaration(self):
        result = subprocess.run([sys.executable, str(self.repo / 'util/sdlc.py'),
    ...
    def fixture_child(self, root, arguments, *, boundary=None, cwd=None, native_launcher=False):
        ...
        launcher = (['node', str(REPOSITORY / 'util/runRepositoryPython.mjs')]
        ...
        if boundary:
            ...
            command = [*launcher, str(root / 'util/sdlc.py'), *arguments]
    ...
    def test_real_launcher_selects_utf8_without_replacing_stream_override(self):
        ...
        command = ['node', str(REPOSITORY / 'util/runRepositoryPython.mjs'), '-c', code, 'space and;literal']
    ...
    def test_real_cli_retains_unicode_child_failure(self):
        ...
        environment = dict(os.environ, PYTHONUTF8='0')
        environment.pop('PYTHONIOENCODING', None)
        result = subprocess.run(['node', str(REPOSITORY / 'util/runRepositoryPython.mjs'),
            str(self.repo / 'util/sdlc.py'), 'verify'], env=environment, capture_output=True)
...
from pathlib import Path
from unittest.mock import patch
sys.path.insert(0, {str(REPOSITORY / 'util')!r})
...
def protocol(arguments, **kwargs):
    ...
    if arguments[1:4] == ['issue', 'view', '123']:
        ...
        assert arguments[1:] == ['repo', 'view', 'https://github.com/example/service.git', '--json', 'nameWithOwner']
...
with patch.object(sdlc, 'git_output_bytes', return_value=b'https://github.com/example/service.git'), patch.object(sdlc, 'require_command', return_value='gh'), patch.object(sdlc, 'run', side_effect=protocol):
...
'''
    ...
    def test_real_cli_escapes_explicit_legacy_streams_without_changing_evidence(self):
        ...
        for encoding in ('cp1252:strict', 'ascii:ignore'):
            with self.subTest(encoding=encoding):
                result = subprocess.run(['node', str(REPOSITORY / 'util/runRepositoryPython.mjs'),
                    str(self.repo / 'util/sdlc.py'), 'verify'],
    ...
    def test_native_current_directory_collision_starts_no_child(self):
        ...
        result = subprocess.run([sys.executable, str(self.repo / 'util/sdlc.py'), 'verify'], capture_output=True)
        self.assertNotEqual(result.returncode, 0)
    ...
    def test_runner_termination_preserves_prior_checkpoint_and_unfinished_child(self):
        ...
        runner = subprocess.Popen([sys.executable, str(self.repo / 'util/sdlc.py'), 'verify'],
    ...
    def test_native_git_scope_selects_new_receipt_schema_and_process_fixture(self):
        ...
        for relative in ('.sdlc/schemas/verification-run.schema.json',
            ...
            with self.subTest(path=relative):
                ...
                result = subprocess.run(['node', str(self.repo / 'util/selectPullRequestChecks.mjs'),
    ...
    def test_stop_hook_requests_one_continuation_then_honest_blocker(self):
        self.begin_task()
        command = [sys.executable, str(self.repo / 'util/sdlc_stop_gate.py')]
    ...
    def test_stop_hook_accepts_current_r0_without_full_profile(self):
        self.begin_task(); self.verify_task()
        result = subprocess.run([sys.executable, str(self.repo / 'util/sdlc_stop_gate.py')],
    ...
    def test_native_hook_preserves_repository_root_arguments_stdin_and_exit_status(self):
        ...
        (self.repo / 'util/runRepositoryPython.mjs').write_text(
        ...
        for exit_code in (0, 2):
            with self.subTest(exit_code=exit_code):
                ...
                result = subprocess.run(invocation,cwd=self.repo/'util',input=json.dumps(hook_input),
                ...
                self.assertEqual(payload['args'], ['util/sdlc_stop_gate.py'])
                self.assertEqual(payload['input'], hook_input)

    def test_unknown_end_command_does_not_execute_a_transition(self):
        result = subprocess.run([sys.executable, str(self.repo / 'util/sdlc.py'), 'end'],
