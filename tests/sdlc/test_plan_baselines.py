...
class NativePlanLifecycleTests(unittest.TestCase):
    ...
    def test_real_cli_uses_approved_python_and_preserves_initial_ownership(self):
        shutil.copytree(REPOSITORY / 'util', self.repo / 'util', ignore=shutil.ignore_patterns('__pycache__'))
        config_before = (self.repo / '.git/config').read_bytes()
        command = [sys.executable, '-B', '-X', 'utf8', str(self.repo / 'util/sdlc.py'), 'begin', 'native-plan-fixture',
