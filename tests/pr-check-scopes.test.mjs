...
const SCOPES = ["sdlc", "application"];

test("application-only PRs do not unconditionally launch the SDLC control matrix", () => {
  ...
  expect(workflow.jobs.scope.steps.at(-1).run).toBe(
    "node util/selectPullRequestChecks.mjs --scope sdlc",
  );
});
...
describe("native Git PR check selection", () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "webvowl-pr-checks-"));
    environment = {
      ...Object.fromEntries(
    ...
    write("package.json", '{"name":"commonjs-fixture"}\n');
    ...
    write(
      "util/selectPullRequestChecks.mjs",
      readFileSync(
        new URL("../util/selectPullRequestChecks.mjs", import.meta.url),
        "utf8",
      ),
    );
    selectorPath = join(root, "util/selectPullRequestChecks.mjs");
    base = commit(["package.json", "util/selectPullRequestChecks.mjs"]);
  });
  ...
  afterEach(() => {
    ...
    if (
      ...
      !basename(target).startsWith("webvowl-pr-checks-")
    ) {
  ...
  test.each([
    ...
    ["src/owl2vowl/converter.js", ["application"]],
    ["src/app/main.js", ["application"]],
    ["package-lock.json", ["sdlc", "application"]],
    [".node-version", ["sdlc", "application"]],
    ["tests/pr-check-scopes.test.mjs", ["sdlc", "application"]],
    ["util/_configuration_transaction.py", ["sdlc"]],
    ["util/test_sdlc_skill_activation.py", ["sdlc"]],
    ["util/zip.mjs", ["application"]],
    ["vite.config.mjs", ["application"]],
    [".npmrc", ["sdlc", "application"]],
    [".sdlc/tmp/probe.json", []],
  ...
  test("a plan plus its accepted baseline does not select unrelated checks", () => {
    const paths = [
      "docs/plans/next-change.md",
  ...
  test("mixed changes take the union of their applicable checks", () => {
    const paths = ["util/set_up_sdlc.py", "src/app/main.js", "README.md"];
    paths.forEach((path) => write(path));
    commit(paths);
    expectSelection(["sdlc", "application"]);
  });
  ...
  test("unusual filenames are passed through native Git without line parsing", () => {
    const path =
      process.platform === "win32"
        ? "src/fixture/quoted ' and café.js"
        : "src/fixture/quoted ' and café\nline.js";
    write(path);
    commit([path]);
    expectSelection(["application"]);
  });
  ...
  test("a relevant input after more than 3000 other files is still selected", () => {
    ...
    paths.push("src/fixture/last-relevant-input.js");
    paths.forEach((path) => write(path));
  ...
  test("a main push compares its before SHA", () => {
    write("docs/sdlc/changed.md");
    commit(["docs/sdlc/changed.md"]);
    expectSelection(["sdlc"], { eventName: "push" });
  });
